...
export default function DashboardHanseniase({ pacientes }) {
  ...
  const dadosCategorias = useMemo(() => {
    ...
    return [
      ...
      { name: "Atenção Especializada", value: especializadas },
    ].filter((d) => d.value > 0);
  }, [pacientes]);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
        <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-slate-100 p-4 sm:p-6 flex flex-col">
          <div className="mb-2">
            <h3 className="text-base sm:text-lg font-bold text-slate-800">
              Distribuição por UBS (Top 5)
            </h3>
            <p className="text-xs sm:text-sm text-slate-500 mt-0.5">
              Áreas com mais casos de Hanseníase
            </p>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 sm:p-6 flex flex-col">
          <h3 className="text-base sm:text-lg font-bold text-slate-800 mb-4 flex items-center gap-2">
            <ShieldAlert className="size-5 text-violet-600" /> Nível de Atenção
          </h3>
                ...
                return (
                  ...
                  >
                    <td className="px-4 py-3 font-medium text-slate-800 flex items-center gap-2 whitespace-nowrap">
                      <div
                        className={`shrink-0 w-8 h-8 rounded-full flex items-center justify-center font-bold ${isEspecializada ? "bg-red-100 text-red-700" : "bg-violet-100 text-violet-700"}`}
                      >
                        {isEspecializada ? "H" : "U"}
                      </div>
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <span
                        className={`px-2 py-1 rounded-md text-[10px] sm:text-xs font-bold inline-block ${isEspecializada ? "bg-red-50 text-red-600 border border-red-200" : "bg-violet-50 text-violet-600 border border-violet-200"}`}
                      >
                        {isEspecializada
