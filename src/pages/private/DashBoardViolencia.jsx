...
export default function DashboardViolencia({ pacientes }) {
  ...
  // 3. Gráfico de Rosca: Perfil de Atendimento (Atenção Básica vs Especializada/Hospitalar)
  const dadosCategorias = useMemo(() => {
    ...
    pacientes.forEach((p) => {
      ...
      // Classifica como Atenção Básica se for UBS/POSTO/ESF, senão é Hospital/CAPS/Delegacia
      if (
        nome.includes("UBS") ||
        nome.includes("POSTO") ||
        nome.includes("ESF")
    ...
    return [
      ...
      { name: "Acolhimento Hospitalar/Especializado", value: especializadas },
    ].filter((d) => d.value > 0);
  }, [pacientes]);
    ...
    <div className="space-y-4 sm:space-y-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        <KpiCard
          ...
          color="bg-pink-500"
          bgLight="bg-pink-50"
          subtitle="Rede de apoio acionada"
        />
        <KpiCard
          title="Unidade com Maior Fluxo"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
        <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-slate-100 p-4 sm:p-6 flex flex-col">
          <div className="mb-2">
            ...
            <p className="text-xs sm:text-sm text-slate-500 mt-0.5">
              Locais com maior volume de notificações registradas
            </p>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 sm:p-6 flex flex-col">
          <h3 className="text-base sm:text-lg font-bold text-slate-800 mb-4 flex items-center gap-2">
            <ShieldAlert className="size-5 text-rose-600" /> Perfil da Rede
          </h3>
          ...
          <table className="w-full text-left text-sm text-slate-600 min-w-[600px]">
            ...
            <tbody>
              {listaUnidades.map((unidade, idx) => {
                const nomeUbs = (unidade.nm_ubs || "").toUpperCase();
